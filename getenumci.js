export const getenumci = `getenumCI <- function(veris, 
                      enum, 
                      by=NULL,
                      na.rm = NULL, 
                      unk=FALSE, 
  ...
  #lifecycle::deprecate_warn("version 2.3.2.0XX", "verisr::getenumCI2023()", "verisr::getenumCI2024()")
  ...
  if (length(ci.method) > 0 && !ci.method %in% c("mcmc", "bootstrap", "bayes", "FALSE")) {
  ...
  if (ci.method) {
    stop("ci.method not supported until binom, infer, and/or brms are supported by webr.");
  }
...
#      }
    ...
    if (length(ci.method) > 0 && ci.method == "bayes") {
      ...
      # removed this code as it messed with webr and wasn't likely to be supported any time soon.
    }
